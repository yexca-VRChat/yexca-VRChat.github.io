...
| **属性**                           | **功能**                                                     |
| :--------------------------------- | :----------------------------------------------------------- |
| **Duration(持续时间)**             | 粒子系统运行的时间长度，或者说周期                           |
...
| **Start Speed(初始速度)**          | 每个粒子在适当方向的初始速度                                 |
...
| **Gravity Modifier(重力修改器)**   | 缩放`Physics`窗口`(Edit-Project Settings-Physics)`中设置的重力值。值为零会关闭重力 |
...
| **Play on Awake(唤醒时播放)**      | 如果启用此属性，则粒子系统会在创建对象时自动启动             |
